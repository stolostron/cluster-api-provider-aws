...
func TestReconcileBucket(t *testing.T) {
	...
	t.Run("creates_bucket_with_single_additional_iam_role", func(t *testing.T) {
		expectedBucketName := "test-bucket"
		expectedRoleName := "karpenter-nodes"
		...
		s3Mock.EXPECT().PutBucketPolicy(gomock.Any(), gomock.Any()).Do(func(ctx context.Context, input *s3svc.PutBucketPolicyInput, optFns ...func(*s3svc.Options)) {
			...
			if !strings.Contains(policy, expectedRoleName) {
				t.Errorf("Policy should reference additional role %q, got: %v", expectedRoleName, policy)
			}
			...
			if !strings.Contains(policy, fmt.Sprintf("arn:aws:iam::foo:role/%s", expectedRoleName)) {
				t.Errorf("Expected arn to contain the additional role principal; got: %v", policy)
			}
	...
	t.Run("creates_bucket_with_multiple_additional_iam_roles", func(t *testing.T) {
		expectedBucketName := "test-bucket"
		...
		s3Mock.EXPECT().PutBucketPolicy(gomock.Any(), gomock.Any()).Do(func(ctx context.Context, input *s3svc.PutBucketPolicyInput, optFns ...func(*s3svc.Options)) {
			...
			// Verify both roles are in policy
			for _, roleName := range []string{"karpenter-nodes", "custom-workload"} {
				if !strings.Contains(policy, roleName) {
					t.Errorf("Policy should reference additional role %q, got: %v", roleName, policy)
				}
			}
	...
	t.Run("ignores_additional_iam_roles_when_presigned_urls_enabled", func(t *testing.T) {
		...
		s3Mock.EXPECT().PutBucketPolicy(gomock.Any(), gomock.Any()).Do(func(ctx context.Context, input *s3svc.PutBucketPolicyInput, optFns ...func(*s3svc.Options)) {
			...
			// Should NOT contain additional role when presigned URLs are enabled
			if strings.Contains(policy, "should-be-ignored") {
				t.Errorf("Policy should not reference additional roles when presigned URLs are enabled, got: %v", policy)
			}
